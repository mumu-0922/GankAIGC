...
- `cd package/backend; python -m pytest -q` 使用临时 SQLite 数据库运行 pytest 测试。
...
不要提交 `.env`、`.env.docker`、API Key、数据库文件或日志。生产部署必须替换默认 `SECRET_KEY` 和 `ADMIN_PASSWORD`，配置有效的模型 API 凭据，并将 `.env.docker.example` 作为文档模板，而不是密钥存储文件。
