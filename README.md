...
当前稳定主线聚焦「账号注册 + 用户登录 + 邀请码注册 + 兑换码充值额度 + 降 AI 工作台」。文档排版相关代码仍保留在后端实验模块中，但不是当前主流程功能。
...
- 额度体系：管理员创建兑换码，用户在前台兑换额度；平台模式按字符折算额度，1000 个非空白字符约消耗 1 额度。
...
2. 在「账号额度」中创建注册邀请码。
3. 用户通过邀请码注册并登录。
4. 管理员创建额度兑换码，用户在前台兑换额度。
5. 用户进入工作台，选择平台额度模式或自带 API 模式，提交论文文本。
...
- 账号额度：管理用户、邀请码、额度兑换码和用户自带 API 摘要。
